...
CsgB_logos <- lapply(CsgA_regions, function(i)
  plot_logo_region(seq_path = seq_path_CsgB, region = i))

grid.newpage()
grid.draw(arrangeGrob(grobs = c(lapply(1L:5, function(i)
  textGrob(paste0("R", i), vjust = -4, gp=gpar(fontsize=18))),
  CsgA_logos), ncol = 2, as.table = FALSE, widths = c(0.02, 0.98)))

@

AmyloGram was used to assign amino acids where assigned to 6 groups. It used eleven combinations of physicochemical properties to create the best performing encoding. Amino acids where divided in subgroups:
I) lowest propensity to form β-sheets
II) the most hydrophilic, includes two strongly basic amino acids, and highly flexible
III) strongly hydrophobic, highest propensity to form β-sheets
IV) aromatic properties, the most hydrophilic, the least flexible, highest propensity to form β-sheets
...
grid.draw(arrangeGrob(grobs = c(lapply(1L:5, function(i)
  textGrob(paste0("R", i), vjust = -4, gp=gpar(fontsize=18))),
  CsgB_logos), ncol = 2, as.table = FALSE, widths = c(0.02, 0.98)))
